...
class AgentQECollaboration:
    def __init__(self, config_path_param, input_path: Optional[str] = None) -> None:
        ...
        self.task_prompt_path = Path(
            config_path_param.parent.parent / "agent_framework/prompts/task_prompts/SauceLab_Auto_Task.txt"
        )
        ...
        self.read_agent = self.factory.create_fs_read_agent(
            file_name=input_path, input_dir="AIOutput", output_dir="AI_Auto_Repos"
        )
        self.playwright_dev_agent = self.factory.create_playwright_dev_agent(
            input_file=input_path, output_dir="AI_Auto_Repos", target_dir="AIOutput"
        )
        ...
        prompt = load_prompt("SauceLab_Auto_Task", "task_prompts")
        input_add = load_input_path(input_path, "AIOutput")
        logger.info(f"input_path_qe: {input_add}")
        logger.info(f"AI_Output_qe: {self.output_dir}")

        self.task_prompt = sanitize_prompt(
            prompt.replace("{Manual_Path}", str(input_path))
            .replace("{AI_Auto_Repo}", str(self.output_dir))
        )

    async def run_qe_team(self):
        try:
